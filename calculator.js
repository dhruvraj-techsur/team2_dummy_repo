...
async function promptForOperation(rl) {
  while (true) {
    const input = await askQuestion(
      rl,
      `Choose operation (${operationNames.join(', ')}): `
    );
    const op = input.trim().toLowerCase();
    if (operations[op]) {
      return op;
    }
    console.log(`Error: Invalid operation. Please choose from: ${operationNames.join(', ')}`);
  }
}
...
async function main() {
  ...
  try {
    ...
    while (true) {
      ...
      if (op === 'divide' && b === 0) {
        console.log('Error: Division by zero is not allowed. Please enter a non-zero second number.');
        continue;
      }
      break;
    }
